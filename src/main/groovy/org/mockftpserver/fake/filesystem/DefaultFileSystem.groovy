...
class DefaultFileSystem implements FileSystem {
    ...
    public List listFiles(String path) {
        ...
        if (PatternUtil.containsWildcards(name)) {
            def pattern = PatternUtil.convertStringWithWildcardsToRegex(name)
            def parent = getParent(path)
            def files = fileForPath(parent).listFiles()
            if (files != null) {
                files.each {file ->
    ...
    public List listNames(String path) {
        ...
        if (PatternUtil.containsWildcards(name)) {
            def parent = getParent(path)
            def filenames = fileForPath(parent).list()
            def list = (filenames == null) ? [] : filenames as List

            def pattern = PatternUtil.convertStringWithWildcardsToRegex(name)
            return list.findAll { it =~ pattern }
        }
        else {
